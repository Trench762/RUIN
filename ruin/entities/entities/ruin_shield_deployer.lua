...
function ENT:PhysicsCollide( data, phys )
    -- This makes sure that split second double collisions dont result in it spawning multiple shields.
    if(self.deployed == false) then
        ParticleEffect("vortigaunt_glow_beam_cp1b", self:GetPos(), self:GetAngles())
        self:Remove()
        if SERVER then 
            local shield = ents.Create("ruin_force_shield")
            local pos = self:GetPos()
            if pos.z > Entity(1):GetPos().z then
                if !IsValid(Entity(1)) then return end
                pos.z = Entity(1):GetPos().z
            end
